...
var AUTH = (function () {
  ...
  var SUPA_HDR = { 'Content-Type': 'application/json', 'apikey': SUPA_KEY, 'Authorization': 'Bearer ' + SUPA_KEY };

  // ── Temp password held in memory only during onboarding ──
  // Never written to localStorage — cleared after finalize()
  var _tempPass = '';

  // ── OTP wiring flag — prevent double event listener attach ──
  var _otpWired = false;
  ...
  var validators = {
    ...
    6: function () {
      ...
      _tempPass = pass;   // hold in memory for verification check
    ...
    8: function () {
      // Hard gate — OTP auto-submits via _autoVerifyOTP(), CTA is hidden.
      // This only fires if someone somehow triggers Next manually.
      showErr('verifyError', '⚠️ Enter the 6-digit code from your email.');
      return 'Enter OTP.';
    }
  };
  ...
  /* ─────────────────────────────────────────────────────────
     ...
     ───────────────────────────────────────────────────────── */
  function finalize() {
    _tempPass = ''; // clear from memory — never needed after this point
  ...
  /* ─────────────────────────────────────────────────────────
     OTP SEND & VERIFY — Supabase email OTP
     ─────────────────────────────────────────────────────────
     Supabase dashboard → Auth → Email Templates → "Confirm signup"
     Replace the confirmation URL with just: Your code: {{ .Token }}
     ───────────────────────────────────────────────────────── */
  function sendOTP() {
    ...
    fetch(SUPA_URL + '/auth/v1/otp', {
      ...
      console.warn('[AnthrosAI] OTP send error', err);
      showErr('verifyError', '⚠️ Could not send code — tap Resend.');
    });
  }

  function _wireOTPBoxes() {
    if (_otpWired) {
      // Already wired — just clear values and focus first box
      _clearOTPBoxes();
      setTimeout(function() { var b = $('otp0'); if (b) b.focus(); }, 150);
      return;
    }
    _otpWired = true;
    ...
    // Paste handler on container — distributes pasted digits across boxes
    var container = $('otpBoxes');
    if (container) {
  ...
  function _autoVerifyOTP() {
    var code = _getOTPValue();
    if (code.length !== 6) return;

    // Disable boxes during network call
    for (var i = 0; i < 6; i++) { var b = $('otp' + i); if (b) b.disabled = true; }
    showErr('verifyError', '');
    ...
    .then(function(data) {
      if (data.access_token) {
        // ✅ Correct code — enter the app
        window._emailVerified = true;
        window._supaAccessToken = data.access_token;
        try { localStorage.setItem('anthros_email_verified', '1'); } catch(e) {}
        _tempPass = '';
        var successEl = $('verifySuccess');
        if (successEl) successEl.style.display = 'block';
        setTimeout(function() { finalize(); }, 1000);
      } else {
        // ❌ Wrong code — shake, clear, focus back
        for (var i = 0; i < 6; i++) { var b = $('otp' + i); if (b) b.disabled = false; }
        _shakeOTPBoxes();
        setTimeout(function() {
          _clearOTPBoxes();
          var first = $('otp0');
        ...
        var msg = ((data.error_description || data.msg || '')).toLowerCase();
        if (msg.indexOf('expired') > -1 || msg.indexOf('otp') > -1) {
          showErr('verifyError', '⏱ Code expired — tap Resend Code for a new one.');
        } else {
          showErr('verifyError', '❌ Wrong code — check your email and try again.');
        }
      }
    })
    .catch(function() {
      for (var i = 0; i < 6; i++) { var b = $('otp' + i); if (b) b.disabled = false; }
      showErr('verifyError', '⚠️ Network error — check your connection.');
    });
  }
  ...
  /* ─────────────────────────────────────────────────────────
     RESEND OTP — 60s cooldown, calls /auth/v1/resend
     ───────────────────────────────────────────────────────── */
  function resendEmail() {
    ...
    .then(function() {
      _clearOTPBoxes();
      setTimeout(function() { var b = $('otp0'); if (b) b.focus(); }, 100);
      _startResendCooldown(60);
    })
    .catch(function() {
      if (btn) { btn.disabled = false; btn.textContent = 'Resend Code'; }
      showErr('verifyError', '⚠️ Could not resend — check your connection.');
    });
  }

  function _startResendCooldown(secs) {
    var btn = $('resend-email-btn');
    var cdEl = $('resendCooldown');
    var secsEl = $('cooldownSecs');
    if (btn) { btn.disabled = true; btn.textContent = 'Resend Code'; }
